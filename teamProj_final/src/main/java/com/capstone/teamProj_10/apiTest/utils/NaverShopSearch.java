...
public class NaverShopSearch {
    ...
    public String search(String query) {    // query를 파라미터로 네이버 쇼핑 API를 사용하여 검색
        ...
        headers.add("X-Naver-Client-Secret", "4ws8LoYgtw");     // 네이버 API 인증 정보
        String body = "";
    ...
    public List<ItemDto> fromJSONtoItems(String result) {
        ...
        for (int i = 0; i < items.length(); i++) {
            ...
            ItemDto itemDto = new ItemDto(itemJson);        // JSONObject를 사용하여 itemDto 생성
            itemDtoList.add(itemDto);                       // 생성된 itemDto 객체를 List에 추가
        }
